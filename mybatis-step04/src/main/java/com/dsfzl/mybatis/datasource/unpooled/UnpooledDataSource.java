...
public class UnpooledDataSource implements DataSource {
    ...
    static {
        // 获取已注册的驱动程序
        // 通过遍历DriverManager中的驱动程序，
        // 可以了解当前JVM中已注册的所有JDBC驱动程序。
     ...
     * 初始化驱动
     */
    //如果两个线程几乎同时检查驱动程序是否已注册并发现未注册，它们都可能会尝试注册同一个驱动程序。
    private synchronized void initializerDrivers() throws SQLException {
        if (!registeredDrivers.containsKey(driver)) {
            try {
                // 通过 driverClassLoader 找到指定类
                Class<?> driverType = Class.forName(driver, true, driverClassLoader);
                // 通过 newInstance 实例化驱动
