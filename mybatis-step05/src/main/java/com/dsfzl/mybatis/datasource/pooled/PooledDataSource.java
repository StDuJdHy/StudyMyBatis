...
import com.dsfzl.mybatis.datasource.unpooled.UnpooledDataSource;
...
public class PooledDataSource implements DataSource {
    ...
    protected void pushConnection(PooledConnection connection) throws SQLException {
        synchronized (state) {
            ...
            // 判断连接是否有效
            // 判断原因：如果连接已经被关闭或者由于某种原因变得不可用，
            // 将其返回到连接池可能会导致后续使用该连接的代码出错。因此，在将连接返回到连接池之前，需要验证其有效性
            if (connection.isValid()) {
                ...
                if (state.idleConnections.size() < poolMaximumIdleConnections && connection.getConnectionTypeCode() == expectedConnectionTypeCode) {
                    ...
                    // 实例化一个新的DB连接，加入到空闲idle列表
                    // 这里把当前正在使用的数据库连接被重新包装成一个新的 PooledConnection 对象，
                    // 并返回到连接池的空闲连接列表中。这样，这个连接可以再次被应用程序借用，而不需要重新创建一个新的数据库连接。
